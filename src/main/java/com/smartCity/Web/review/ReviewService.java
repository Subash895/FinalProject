...
public class ReviewService {
    ...
    public ReviewService(ReviewRepository reviewRepository, UserRepository userRepository,
            BusinessRepository businessRepository, PlaceRepository placeRepository, CityRepository cityRepository,
            NewsRepository newsRepository, EmailNotificationService emailNotificationService) {
    ...
    public Review createOrUpdateReview(ReviewDtos.ReviewRequest request, Authentication authentication) {
    ...
    public Review updateReview(Long reviewId, ReviewDtos.ReviewRequest request, Authentication authentication) {
        JwtUserPrincipal principal = extractPrincipal(authentication);
        User user = userRepository.findById(principal.id())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.UNAUTHORIZED, "User not found"));
    ...
    private boolean canSubmitReview(User user) {
        return user.getRole() == Role.USER || user.getRole() == Role.BUSINESS || user.getRole() == Role.ADMIN;
    }
    ...
    private Review findManagedReview(Long reviewId, User user) {
        ...
        return reviewRepository.findByIdAndUserId(reviewId, user.getId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.FORBIDDEN, "Review access denied"));
    }
    ...
    private String describeTarget(Review review) {
        ...
        case BUSINESS -> businessRepository.findById(review.getTargetId())
